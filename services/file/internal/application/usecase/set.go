...
type Set struct {
	objects ports.FileObjectRepository
	links   ports.FileLinkRepository
...
func (s *Set) InitUpload(ctx context.Context, p InitUploadParams) (*model.FileObject, *UploadInfo, error) {
	if p.MimeType == "" {
...
func (s *Set) ConfirmUpload(ctx context.Context, p ConfirmUploadParams) (*model.FileObject, error) {
...
func (s *Set) GetDownloadURL(ctx context.Context, fileID uuid.UUID) (string, time.Time, error) {
...
func (s *Set) Link(ctx context.Context, p LinkParams) (*model.FileLink, error) {
	f, err := s.objects.GetByID(ctx, p.FileID)
	if err != nil {
		return nil, err
	}
...
func (s *Set) Unlink(ctx context.Context, fileID uuid.UUID, usageType model.FileUsageType, ownerID uuid.UUID) (bool, error) {
	return s.links.Delete(ctx, fileID, usageType, ownerID)
}

func (s *Set) GetFile(ctx context.Context, id uuid.UUID) (*model.FileObject, error) {
	return s.objects.GetByID(ctx, id)
}

func (s *Set) DeleteFileIfUnlinked(ctx context.Context, fileID uuid.UUID) (*model.FileObject, error) {
	f, err := s.objects.GetByID(ctx, fileID)
	if err != nil {
		return nil, err
	}
...
func (s *Set) RunCleanupBatch(ctx context.Context, limit int) (CleanupResult, error) {
	if limit <= 0 {
		limit = 100
	}
