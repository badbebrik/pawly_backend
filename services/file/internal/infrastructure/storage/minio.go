...
func NewMinio(cfg *config.Config) (*MinioClient, error) {
	...
	minioEndpoint, minioSecure, err := normalizeEndpoint(cfg.MinioEndpoint, cfg.MinioUseSSL, cfg.MinioBucket)
	if err != nil {
	...
	if cfg.MinioPublicEndpoint != "" {
		presignEndpoint, presignSecure, err = normalizeEndpoint(cfg.MinioPublicEndpoint, cfg.MinioUseSSL, cfg.MinioBucket)
		if err != nil {
	...
	uploadTTL, err := secondsToDuration(cfg.UploadURLTTLSeconds, "upload ttl")
	if err != nil {
		return nil, err
	}
	downloadTTL, err := secondsToDuration(cfg.DownloadURLTTLSeconds, "download ttl")
	if err != nil {
		return nil, err
	}
...
func (m *MinioClient) UploadTTL() time.Duration   { return m.uploadTTL }
func (m *MinioClient) DownloadTTL() time.Duration { return m.downloadTTL }

func secondsToDuration(sec int, label string) (time.Duration, error) {
	if sec <= 0 {
